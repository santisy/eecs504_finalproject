...
D1 = single(D1); D2 = single(D2);
% normaliz SIFT descriptor 
for i = 1:size(F1,2)
    D1(:,i) = D1(:,i)/norm(D1(:,i));
end
for i = 1:size(F2,2)
...
for i = 1:n1
    ...
    xlt = max(1,floor(xc(i) - R1(i) + 0.5));
    ylt = max(1,floor(yc(i) - R1(i) + 0.5));
...
for i = 1:length(R2)
    % Assume we would never cross the boundary
    xlt = max(1,floor(xc(i) - R2(i) + 0.5));
    ylt = max(1,floor(yc(i) - R2(i) + 0.5));
...
for i = 1:rect_num1
    ...
    for j = 1:rect_num2     
        ...
        [~,score] = fastmatch(De1(:,idx1),De2(:,idx2),thresh);
        score = sort(score,'ascend');
	if (length(score) < 15)
	    cost(i,j) = inf;
	else
            cost(i,j) = mean(score(1:15));
        end
        %cost(i,j) = cost(i,j) + 0.8*norm(V1(:,i)-V2(:,j));
        %cost(i,j) = mean(score1(1:5))+mean(score2(1:5));
    end
end
%--------------------------------------------------------------------------
%% Show match
