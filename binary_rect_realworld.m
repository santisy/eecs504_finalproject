% adjusted for real-world dataset
clc,clear
close all
%load('002_circle.mat');
bw_im = zeros(540,540,5);
for i = 1:5
...
for i = 1:5
    ...
    if length(unique_label)>8
        ...
        unique_label = unique_label(index_descend(1:4));
    end
    rect_tempRecord = zeros(length(unique_label), 4);
    for j = 1:length(unique_label)
        [lx, ly] = ind2sub([540,540], find(label_bw==unique_label(j)));
        point_l = [lx, ly];
